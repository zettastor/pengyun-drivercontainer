...
$java -noverify -Dmetric.enable=$metric_enable -Dmetric.enable.profiles=$metric_enable_profiles -server -Xms$xms -Xmx$xmx -XX:+UseG1GC -XX:MaxGCPauseMillis=$max_gc_pause_ms -XX:GCPauseIntervalMillis=$gc_pause_interval_ms -XX:ParallelGCThreads=$parallel_gc_threads -cp "$ROOTPATH:$ROOTPATH/lib/*:$ROOTPATH/config" py.drivercontainer.Launcher  $SERVICE_RUNNING_PATH 2>&1 > /dev/null
